...
import 'package:deliva_pa/home_screen/dpc_deliver_package_to_receiver.dart';
...
class DPCPackageDetail extends StatefulWidget {
  final String status;
  final String title;
  final int requestId;

  DPCPackageDetail(this.title, this.requestId, this.status, {Key key})
      : super(key: key);

  @override
  _DPCPackageDetailState createState() => _DPCPackageDetailState();
}

class _DPCPackageDetailState extends State<DPCPackageDetail> {
                                        ...
                                        child: GestureDetector(
                                          onTap: () async {
                                            print('Varify Package');
                                              //showVerifyAlert();
                                            _navigateToDeliverToReceiver(widget.title,widget.requestId);
                                          },
                                          child: Container(
                                            //width: 100.0,
                                            height: 50.0,
                                            ...
                                            child: Center(
                                              child: Text(
                                                '${StringValues.DeliverPackage.toUpperCase()}',
                                                style: TextStyle(
                                                  fontSize: 20.0,
...
/*  Future showVerifyAlert() async {
  ...
  Future _navigateToDeliverToReceiver(
      String title,
      int requestId
      ) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => DPCDeliverPackageToReceiver(title, requestId,widget.status)),
    );
  }
